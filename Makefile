...
# Full debug mode - all modules enabled
debug: CFLAGS += -g -DDEBUG=1 -DDEBUG_LEVEL=DEBUG_INFO
debug: re

# Trace mode - maximum verbosity
trace: CFLAGS += -g -DDEBUG=1 -DDEBUG_LEVEL=DEBUG_TRACE
trace: re

# Debug specific modules
debug-parser: CFLAGS += -g -DDEBUG=1 \
	-DDEBUG_PARSER=1 -DDEBUG_EXEC=0 -DDEBUG_BUILTIN=0 \
	-DDEBUG_SIGNAL=0 -DDEBUG_REDIR=0 -DDEBUG_PIPE=0 \
	-DDEBUG_ENV=0 -DDEBUG_MAIN=0 -DDEBUG_PATH=0
debug-parser: re

debug-exec: CFLAGS += -g -DDEBUG=1 \
	-DDEBUG_PARSER=0 -DDEBUG_EXEC=1 -DDEBUG_BUILTIN=0 \
	-DDEBUG_SIGNAL=0 -DDEBUG_REDIR=1 -DDEBUG_PIPE=1 \
	-DDEBUG_ENV=0 -DDEBUG_MAIN=0 -DDEBUG_PATH=1
debug-exec: re

debug-builtin: CFLAGS += -g -DDEBUG=1 \
	-DDEBUG_PARSER=0 -DDEBUG_EXEC=0 -DDEBUG_BUILTIN=1 \
	-DDEBUG_SIGNAL=0 -DDEBUG_REDIR=0 -DDEBUG_PIPE=0 \
	-DDEBUG_ENV=1 -DDEBUG_MAIN=0 -DDEBUG_PATH=0
debug-builtin: re

debug-minimal: CFLAGS += -g -DDEBUG=1 -DDEBUG_LEVEL=DEBUG_ERROR
debug-minimal: re
...
supp:
	...
	@echo "}" >> readline.supp
	@echo "{" >> readline.supp
	@echo "   Readline_history" >> readline.supp
	@echo "   Memcheck:Leak" >> readline.supp
	@echo "   ..." >> readline.supp
...
# Norm check
norm:
	@norminette src/ includes/ | grep -v "OK!" || echo "Norm OK!"

# Count lines per function
lines:
	@echo "=== Lines per function ==="
	@find src -name "*.c" -exec awk '/^[a-zA-Z].*\(.*\)$$/{p=1} \
		p && /^{/{c=0; p=0} p{c++} \
		/^}/{if(c>25) print FILENAME":"name" has "c" lines"; p=0} \
		/^[a-zA-Z].*\(.*\)$$/{name=$$0}' {} \;
...
.PHONY: all clean fclean re debug trace debug-parser debug-exec \
		debug-builtin debug-minimal valgrind valgrind-trace supp \
		norm lines test